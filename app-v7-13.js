...
const protocolCategories = {
  "Recover": {
    ...
    purpose:"Restore · Downshift · Support HRV"
  },
  "Perform": {
    ...
    purpose:"Activate · Prime · Energize"
  },
  "Focus": {
    ...
    purpose:"Focus · Composure · Control"
  },
  "Train · Advanced": {
    ...
    purpose:"Build Tolerance · Train Breath Control"
  }
};

const libraryOrder=["Recover","Perform","Focus","Train · Advanced"];

function categoryForProtocol(p){
  return protocolCategories[p.category] || protocolCategories["Recover"];
}
...
function protocolDisplaySummary(p){
  const map={
    ...
    hypoxicTolerance:"4s in · 6s out · hold 6/8/10/12s"
  };
  return map[p.id] || `${p.inhale}s in · ${p.exhale}s out`;
}
...
function protocolBreathRate(p){
  if(p.id==="recovery55") return "~5.5";
  if(p.id==="recoveryDownshift" || p.id==="resonance6" || p.id==="sleepDownshift") return "~6";
...
function protocolHowIcon(p){
  if(p.category==="Recover") return "♡";
  if(p.category==="Perform") return "↗";
  if(p.category==="Focus") return "◎";
  return "△";
}
...
function sessionProgressLabel(){
  ...
  if(!p || !breathRuntime.running) return "Ready";
  const elapsed=Math.max(0,performance.now()-breathRuntime.startedAt);
  const pct=Math.min(1,elapsed/Math.max(1,breathRuntime.durationMs));
  if(p.id==="478") return `${Math.max(1,Math.ceil(pct*6))} / 6 cycles`;
  if(p.id==="hypoxicTolerance") return `Round ${Math.min(4,Math.max(1,Math.ceil(pct*4)))} of 4`;
  if(p.id==="boltReset") return `Round ${Math.min(3,Math.max(1,Math.ceil(pct*3)))} of 3`;
...
function renderBreathingLibrary(){
  ...
  root.innerHTML=libraryOrder.map(catName=>{
    const meta=protocolCategories[catName];
    const items=protocols.filter(p=>p.category===catName);
    return `<section class="library-category ${meta.className}">
      <div class="library-category-heading">
        <div class="library-category-title">
          <span class="library-category-icon">${meta.icon}</span>
          <div>
            <strong>${meta.label}</strong>
            <small>${meta.tagline}</small>
          </div>
        </div>
        <span class="library-count">${items.length} protocol${items.length===1?"":"s"}</span>
      </div>
      <div class="library-protocol-list">
        ${items.map((p,i)=>`
          <button class="library-protocol-row" data-library-protocol="${p.id}">
            <span class="library-number">${protocols.indexOf(p)+1}</span>
            <span class="library-protocol-copy">
              <strong>${p.name}</strong>
              <small>${protocolDisplaySummary(p)}</small>
            </span>
            <span class="library-duration">${protocolDurationLabel(p)}</span>
  ...
  root.querySelectorAll("[data-library-protocol]").forEach(btn=>{
    btn.addEventListener("click",()=>openProtocolDetail(btn.dataset.libraryProtocol));
  });
}
...
function updateBreatheHeader(){
  ...
  $("#breathCategoryTop").textContent=categoryForProtocol(p).label;
...
function isTabSwipeExcluded(target){
  return !!target.closest(
    '#breathSwipeDeck, button, input, select, textarea, a, .today-widget.dragging, .floating-drag, .modal, .settings-sheet'
  );
}
...
function setBreathPlayer(p, reset=true){
  ...
  const cat=categoryForProtocol(p);
  const player=$("#breathPlayer");
  if(player){
    player.classList.remove("category-recover","category-perform","category-focus","category-train");
    player.classList.add(cat.className);
  }
  $("#breathPurposePill").textContent=`${cat.icon} ${cat.purpose}`;
  $("#breathHowIcon").textContent=protocolHowIcon(p);
  $("#breathRateMetric").textContent=protocolBreathRate(p);
...
function phaseCardsForProtocol(p){
  ...
  return [["Inhale",`${p.inhale}s`],["Exhale",`${p.exhale}s`]];
}

function renderPhaseCards(p){
  const root=$("#breathPhaseCards");
  if(!root)return;
  const cards=phaseCardsForProtocol(p);
  root.innerHTML=cards.map((x,i)=>`
    <div class="breath-phase-card ${i===0?"active":""}">
      <span>${x[0]}</span>
      <strong>${x[1]}</strong>
    </div>`).join("");
}
