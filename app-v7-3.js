...
function wireTodayReorder(){
  ...
  let active=null, startY=0, activated=false, hold=null;

  const moveWidget=(clientY)=>{
    if(!active)return;
    const before=captureWidgetPositions();
    const siblings=[...screen.querySelectorAll(".today-widget")].filter(x=>x!==active);
    for(const target of siblings){
      const r=target.getBoundingClientRect();
      if(clientY < r.top + r.height/2){
        if(target.previousElementSibling!==active) target.before(active);
        animateWidgetReflow(before);
        return;
      }
    }
    const hint=$("#todayEditHint");
    if(hint && active.nextElementSibling!==hint){
      screen.insertBefore(active,hint);
      animateWidgetReflow(before);
    }
  };

  const begin=(widget,clientY)=>{
    active=widget;startY=clientY;activated=false;
    clearTimeout(hold);
    hold=setTimeout(()=>{
      activated=true;
      ...
      widget.animate([
        ...
        {transform:"scale(1.018)",boxShadow:"0 18px 42px rgba(0,0,0,.28)"}
  ...
  const move=(clientY,event)=>{
    if(!active)return;
    if(!activated && Math.abs(clientY-startY)>10){clearTimeout(hold);active=null;return;}
    if(activated){
  ...
  const finish=()=>{
    clearTimeout(hold);
    if(active && activated){
      active.classList.remove("dragging","editing");
      active.animate([{transform:"scale(1.018)"},{transform:"scale(1)"}],{duration:180,easing:"ease-out"});
      persistTodayOrder();
    }
    active=null;activated=false;
  };

  screen.querySelectorAll(".today-widget").forEach(widget=>{
    widget.addEventListener("touchstart",e=>{
      if(e.target.closest("button,input,select,a"))return;
      if(e.touches.length===1) begin(widget,e.touches[0].clientY);
    },{passive:true});
...
function renderProtocols(){
  ...
  $$("[data-dot-index]").forEach(dot=>dot.addEventListener("click",()=>{
    ...
    if(!direction) return;
    animateProtocolChange(direction).then(()=>selectProtocolByIndex(next));
  }));
...
function animateProtocolChange(direction){
  const player=$("#breathPlayer");
  if(!player) return Promise.resolve();
  const outX=direction>0?-42:42;
  const inX=-outX;
  return player.animate([
    {transform:"translateX(0)",opacity:1},
    {transform:`translateX(${outX}px)`,opacity:.2}
  ],{duration:145,easing:"ease-in",fill:"forwards"}).finished
  .catch(()=>{})
  .then(()=>{
    player.style.transform=`translateX(${inX}px)`;
    player.style.opacity=".2";
    requestAnimationFrame(()=>{
      const anim=player.animate([
        {transform:`translateX(${inX}px)`,opacity:.2},
        {transform:"translateX(0)",opacity:1}
      ],{duration:220,easing:"cubic-bezier(.2,.8,.2,1)",fill:"forwards"});
      anim.finished.finally(()=>{player.style.transform="";player.style.opacity="";});
    });
  });
}
function selectAdjacentProtocol(delta){
  const idx=protocols.findIndex(p=>p.id===state.selectedProtocol);
  animateProtocolChange(delta).then(()=>selectProtocolByIndex(idx+delta));
}
...
function wireBreatheSwipe(){
  ...
  player.addEventListener("touchmove",e=>{
    ...
    const t=e.touches[0],dx=t.clientX-breatheSwipeStartX,dy=t.clientY-breatheSwipeStartY;
    if(Math.abs(dx)>Math.abs(dy)*1.2){
      breatheSwipeDx=dx;
      const clamped=Math.max(-70,Math.min(70,dx*.45));
      player.style.transform=`translateX(${clamped}px)`;
      player.style.opacity=String(1-Math.min(.45,Math.abs(clamped)/180));
    }
  },{passive:true});
  player.addEventListener("touchend",()=>{
    ...
    if(Math.abs(dx)>45) selectAdjacentProtocol(dx<0?1:-1);
    else player.animate([{transform:`translateX(${Math.max(-30,Math.min(30,dx*.25))}px)`},{transform:"translateX(0)"}],{duration:160,easing:"ease-out"});
  },{passive:true});
}
