...
function renderBreathingLibrary(){
  ...
  root.querySelectorAll("[data-library-protocol]").forEach(row=>{
    row.addEventListener("click",e=>{
      if(Date.now()<suppressLibraryTapUntil){
        e.preventDefault();
        e.stopPropagation();
        return;
      }
      openProtocolDetail(row.dataset.libraryProtocol);
    });
  });
}
...
$$("[data-tab]").forEach(b=>b.addEventListener("click",()=>{
  ...
  if(target==="breathe"){
    if(breathRuntime.running) stopBreath(false);
    const active=document.querySelector('.screen.active[data-screen]')?.dataset.screen;
    if(active!=="breathe") navigate("breathe");
    showBreatheLibrary();
...
function renderProtocols(){
  ...
  const counter=$("#protocolCounter");
  if(counter) counter.textContent=`${currentIndex+1} / ${protocols.length}`;

  const cards=$("#protocolCards");
  if(cards){
    cards.innerHTML=protocols.map((p,i)=>`<button data-protocol="${p.id}" data-index="${i}" tabindex="-1">${p.name}</button>`).join("");
  }

  const dots=$("#protocolDots");
  if(dots){
    dots.innerHTML=protocols.map((p,i)=>`<button class="protocol-dot ${i===currentIndex?"active":""}" data-dot-index="${i}" aria-label="${p.name}"></button>`).join("");
  }
  ...
  $$("[data-dot-index]").forEach(dot=>dot.addEventListener("click",()=>{
    ...
    if(Number.isFinite(next) && next!==current) selectProtocolByIndex(next);
  }));

  $$("[data-protocol]").forEach(card=>card.addEventListener("click",()=>{
    state.selectedProtocol=card.dataset.protocol;
    saveState();
    openProtocolDetail(card.dataset.protocol);
  }));
}
...
let suppressLibraryTapUntil=0,tabPan=null;
function primaryScreen(n){return document.querySelector(`.screen[data-screen="${n}"]`)}
function activePrimaryName(){return document.querySelector('.screen.active[data-screen]')?.dataset.screen||null}
function tabPanIgnored(t){return !!t.closest('#breathSwipeDeck,input,select,textarea,a,.bottom-nav,.today-widget.dragging,.floating-drag,.modal,.settings-sheet')}
function clearTabPreview(){PRIMARY_TABS.forEach(n=>{const s=primaryScreen(n);if(s){s.classList.remove("tab-pan-preview");s.style.removeProperty("--tab-pan-x")}})}
function renderTabPreview(dx){
 const i=PRIMARY_TABS.indexOf(tabPan?.screen);if(i<0)return;
 const cur=primaryScreen(tabPan.screen),w=Math.max(1,innerWidth),dir=dx<0?1:-1,next=primaryScreen(PRIMARY_TABS[i+dir]);
 if(cur){cur.classList.add("tab-pan-preview");cur.style.setProperty("--tab-pan-x",`${dx}px`)}
 if(next){next.classList.add("tab-pan-preview");next.style.setProperty("--tab-pan-x",`${dx+dir*w}px`)}
}
function wirePrimaryTabSwipe(){
 document.addEventListener("touchstart",e=>{
  if(e.touches.length!==1||tabPanIgnored(e.target)){tabPan=null;return}
  const screen=activePrimaryName();if(!PRIMARY_TABS.includes(screen)){tabPan=null;return}
  const t=e.touches[0];tabPan={x:t.clientX,y:t.clientY,screen,claimed:false,dx:0,dy:0}
 },{passive:true,capture:true});
 document.addEventListener("touchmove",e=>{
  if(!tabPan||e.touches.length!==1)return;
  const t=e.touches[0];tabPan.dx=t.clientX-tabPan.x;tabPan.dy=t.clientY-tabPan.y;
  if(!tabPan.claimed){
   if(Math.abs(tabPan.dy)>14&&Math.abs(tabPan.dy)>Math.abs(tabPan.dx)*1.1){clearTabPreview();tabPan=null;return}
   if(Math.abs(tabPan.dx)>10&&Math.abs(tabPan.dx)>Math.abs(tabPan.dy)*1.1)tabPan.claimed=true;
  }
  if(tabPan.claimed){
   const i=PRIMARY_TABS.indexOf(tabPan.screen);
   if((i===0&&tabPan.dx>0)||(i===PRIMARY_TABS.length-1&&tabPan.dx<0))tabPan.dx*=.28;
   renderTabPreview(tabPan.dx);if(e.cancelable)e.preventDefault()
  }
 },{passive:false,capture:true});
 document.addEventListener("touchend",()=>{
  if(!tabPan){clearTabPreview();return}
  const {dx,dy,screen,claimed}=tabPan,i=PRIMARY_TABS.indexOf(screen),dir=dx<0?1:-1,next=i+dir;
  const change=claimed&&Math.abs(dx)>=Math.min(72,innerWidth*.18)&&Math.abs(dx)>Math.abs(dy)*1.05&&next>=0&&next<PRIMARY_TABS.length;
  clearTabPreview();if(change){suppressLibraryTapUntil=Date.now()+400;navigate(PRIMARY_TABS[next])}tabPan=null
 },{passive:true,capture:true});
 document.addEventListener("touchcancel",()=>{clearTabPreview();tabPan=null},{passive:true,capture:true})
}
...
$("#calculateRun").addEventListener("click",()=>{
  try{
    ...
    $("#runResult").textContent=`Field-test VO₂ estimate: ${v.toFixed(1)} ml/kg/min · ${vo2Tier(v)}`; showToast("VO₂ milestone saved");
...
$("#calculateRockport").addEventListener("click",()=>{
  try{
    ...
    $("#rockportResult").textContent=`Field-test VO₂ estimate: ${v.toFixed(1)} ml/kg/min · ${vo2Tier(v)}`; showToast("VO₂ milestone saved");
