...
let globalSwipeStartX=null;
let globalSwipeStartY=null;
let globalSwipeTarget=null;
let globalSwipeClaimed=false;
let suppressLibraryTapUntil=0;

function activePrimaryScreen(){
  return document.querySelector('.screen.active[data-screen]');
}

function globalSwipeShouldIgnore(target){
  // Let the Breathe detail deck keep its own protocol swipe gesture.
  if(target.closest('#breathSwipeDeck')) return true;

  // Don't hijack form controls, modals, or active Today drag gestures.
  if(target.closest('input,select,textarea,a,.today-widget.dragging,.floating-drag,.modal,.settings-sheet')) return true;
...
function resetGlobalSwipe(){
  globalSwipeStartX=null;
  globalSwipeStartY=null;
  globalSwipeTarget=null;
  globalSwipeClaimed=false;
}

function completeGlobalTabSwipe(dx){
  const screen=activePrimaryScreen();
  if(!screen)return;

  const current=PRIMARY_TABS.indexOf(screen.dataset.screen);
  if(current<0)return;
  ...
  suppressLibraryTapUntil=Date.now()+400;
  navigate(PRIMARY_TABS[next]);
}

function wirePrimaryTabSwipe(){
  // Capture at document level so nested scroll containers/buttons do not swallow
  // the gesture before the app can classify it as horizontal.
  document.addEventListener("touchstart",e=>{
    if(e.touches.length!==1 || globalSwipeShouldIgnore(e.target)){
      resetGlobalSwipe();
      return;
    }

    const t=e.touches[0];
    globalSwipeStartX=t.clientX;
    globalSwipeStartY=t.clientY;
    globalSwipeTarget=e.target;
    globalSwipeClaimed=false;
  },{passive:true,capture:true});

  document.addEventListener("touchmove",e=>{
    if(globalSwipeStartX==null || e.touches.length!==1)return;

    const t=e.touches[0];
    const dx=t.clientX-globalSwipeStartX;
    const dy=t.clientY-globalSwipeStartY;

    // Wait until direction is obvious.
    if(!globalSwipeClaimed){
      if(Math.abs(dy)>22 && Math.abs(dy)>Math.abs(dx)*1.15){
        resetGlobalSwipe();
        return;
      }

      if(Math.abs(dx)>18 && Math.abs(dx)>Math.abs(dy)*1.15){
        globalSwipeClaimed=true;
      }
    }

    if(globalSwipeClaimed && e.cancelable){
      // Once clearly horizontal, prevent Safari/page scrolling from taking it.
      e.preventDefault();
    }
  },{passive:false,capture:true});

  document.addEventListener("touchend",e=>{
    if(globalSwipeStartX==null){
      resetGlobalSwipe();
      return;
    }
    ...
    const dx=touch.clientX-globalSwipeStartX;
    const dy=touch.clientY-globalSwipeStartY;
    const horizontal=
      globalSwipeClaimed &&
      Math.abs(dx)>=55 &&
    ...
    if(horizontal){
      completeGlobalTabSwipe(dx);
    }

    resetGlobalSwipe();
  },{passive:true,capture:true});

  document.addEventListener("touchcancel",resetGlobalSwipe,{passive:true,capture:true});
}
